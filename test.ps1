...
# 7. statusline-command.ps1 runs and produces output (fallback: model defaults to "Claude", bar shows "?%")
$statusRaw = & powershell -NoProfile -Command "echo '{}' | & '$RepoRoot\scripts\statusline-command.ps1'" 2>&1 | Out-String
$esc = [char]27
$statusClean = $statusRaw -replace "$esc\[[0-9;]*m", ""   # strip ANSI color codes before matching
Assert (($statusClean -match "\[Claude\]") -and ($statusClean -match "\?% \| \?k left")) "statusline-command.ps1 emits colored fallback output"
